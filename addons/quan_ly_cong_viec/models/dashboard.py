...
class Dashboard(models.Model):
    ...
    # 🛠️ THÊM TRƯỜNG MỚI: Dự án đã hủy bỏ
    du_an_da_huy = fields.Integer(string="Dự án đã hủy", compute="_compute_tong_quan", store=True)

    @api.depends('so_luong_nhan_vien', 'so_luong_du_an', 'so_luong_cong_viec', 'du_an_hoan_thanh', 
                 'du_an_dang_thuc_hien', 'du_an_chua_bat_dau', 'du_an_tam_dung', 'du_an_da_huy')
    def _compute_tong_quan(self):
        for record in self:
            record.so_luong_nhan_vien = self.env['nhan_vien'].search_count([])
            record.so_luong_du_an = self.env['du_an'].search_count([])
            ...
            # Logic đếm các trạng thái cũ
            record.du_an_hoan_thanh = self.env['du_an'].search_count([('tien_do_du_an', '=', 'hoan_thanh')])
            record.du_an_dang_thuc_hien = self.env['du_an'].search_count([('tien_do_du_an', '=', 'dang_thuc_hien')])
            record.du_an_chua_bat_dau = self.env['du_an'].search_count([('tien_do_du_an', '=', 'chua_bat_dau')])
            record.du_an_tam_dung = self.env['du_an'].search_count([('tien_do_du_an', '=', 'tam_dung')])
            
            # 🛠️ LOGIC ĐẾM: Dự án đã hủy (Tìm theo giá trị trạng thái hủy trong bảng du_an)
            record.du_an_da_huy = self.env['du_an'].search_count([('tien_do_du_an', '=', 'huy_bo')])

            # Tính toán tiến độ trung bình (Bỏ qua các dự án đã hủy để báo cáo chính xác hơn nếu cần)
            du_an_tinh_tien_do = self.env['du_an'].search([('tien_do_du_an', '!=', 'huy_bo')])
            # (Bạn giữ nguyên logic tính toán tiến độ cũ của bạn tại đây nếu có)
